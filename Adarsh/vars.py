...
class Var(object):
    ...
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', '-1580183981'))
    PORT = int(getenv('PORT', 8080))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '0.0.0.0'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = set(int(x) for x in os.environ.get("OWNER_ID", "1001066197625").split())  
    NO_PORT = bool(getenv('NO_PORT', False))
    APP_NAME = None
    OWNER_USERNAME = str(getenv('OWNER_USERNAME', 'username'))
    if 'DYNO' in environ:
        ON_HEROKU = True
    ...
    UPDATES_CHANNEL = str(getenv('UPDATES_CHANNEL', 'SwishMixMusic_bot'))
